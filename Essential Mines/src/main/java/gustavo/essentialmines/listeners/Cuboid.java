...
public class Cuboid implements Iterable<Block>, Cloneable, ConfigurationSerializable {
    ...
    public void setLowerNE(Location location) {
        if (!location.getWorld().getName().equals(this.worldName)) {
            throw new IllegalArgumentException("Location must be in the same world as the Cuboid");
        }
        this.x1 = location.getBlockX();
        this.y1 = location.getBlockY();
        this.z1 = location.getBlockZ();
    }
    public void setUpperSW(Location location) {
        if (!location.getWorld().getName().equals(this.worldName)) {
            throw new IllegalArgumentException("Location must be in the same world as the Cuboid");
        }
